...
public class TraineesController : ControllerBase
{
  // Instance of the service create when the HTTP request is made
    ...
    {
        var traineesVal = await _service.GetAllTraineesService();
    ...
    {
        var traineeDto = await _service.GetTraineeResponseByIdService(id);
        if (traineeDto == null)
            return NotFound();
    ...
    {
        var traineeDto = await _service.CreateTraineeService(trainee);
  ...
  {
    var traineeDto = await _service.UpdateTraineeService(id,trainee);
    if(traineeDto is null)
    {
      return NotFound();
    }
  ...
  public async Task<ActionResult<TraineeResponse>> GetSearch([FromQuery] String search)
      ...
      if(search == null)
    {
      ...
      var traineeDto = await _service.SearchTraineeService(search);
      if (traineeDto == null)
